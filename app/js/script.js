...
$(function () {
  ...
  $('.reded.card-group').on('afterChange', function () {
    ...
    $("#teachersBlock > .teacher_".concat(num)).removeClass('hiddenText');
  });
  ...
  if ($('#calendar')[0]) {
    // Cut the ends
    for (var z = 0; z < $fullCalendar.length; z++) {
      var select = $($fullCalendar[z]).find('.selected-date');
    ...
    for (var i = 0; i < selectedData.length; i++) {
      ...
      if (+parsedDate[0] < d.getMonth() + 1 || +parsedDate[1] < d.getFullYear()) {// let xz = $($('.calendar-body')[i]).find('.calendar__data_date')
    ...
    for (var _i = 0; _i < selectedBody.length; _i++) {
      // Gray out past events
      var parsedDate = selectedBody[_i].getAttribute('data-calendar_body').split('_');

      if (+parsedDate[0] < d.getMonth() + 1 || +parsedDate[1] < d.getFullYear()) {
        // console.log(selectedBody[i])
        $(selectedBody[_i]).find('.calendar__data_date').css('color', 'grey');
      }

      if (+parsedDate[0] == d.getMonth() + 1 && +parsedDate[1] == d.getFullYear()) {
        for (var x = 0; x < $(selectedBody[_i]).children().length; x++) {
          var day = parseInt($($($(selectedBody[_i]).children()[x]).children()[0]).text());

          if (day < d.getDate()) {
            $($($(selectedBody[_i]).children()[x]).children()[0]).css('color', 'grey');
          }
        }
      }
    }
  ...
  $('#about_us .about_us__tabs > .flex-column > .nav-item > .nav-link').on('click', function () {
    // Differences and menthors events switcher
  ...
  $('.calendar__tabs > .flex-column > .nav-item > .nav-link').on('click', function () {
    // CIty events switcher
    if ($(this).hasClass('forChild')) {
  ...
  $.get('https://fluentrussia-metrika.firebaseio.com/profile_1.json', function (data) {
    ...
    console.log("The HR video was viewed ".concat(param_1, " times"));
  });
  $.get('https://fluentrussia-metrika.firebaseio.com/profile_2.json', function (data) {
    param_2 = Object.values(data).length;
    console.log("The Quiz video was viewed ".concat(param_2, " times"));
  });
